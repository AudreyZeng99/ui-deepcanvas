...
export default function MaterialEditor() {
  const { currentProject, createProject, saveProject, markAsDirty, isDirty, validateSave, projects, recordExportedAsset } = useProject();
  ...
  const [aiMiddleMode, setAiMiddleMode] = useState<'preview' | 'library'>('preview');
  const [selectedMaterial, setSelectedMaterial] = useState<{id: number, src: string, prompt: string} | null>(null);

  const materialLibrary = [
    { id: 1, src: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=400&q=80', prompt: '高品质电商海报背景，立体几何图形，柔和光照，C4D渲染' },
    { id: 2, src: 'https://images.unsplash.com/photo-1557683316-973673baf926?w=400&q=80', prompt: '极简主义，流体渐变，抽象艺术，高饱和度，磨砂质感' },
    { id: 3, src: 'https://images.unsplash.com/photo-1558591710-4b4a1ae0f04d?w=400&q=80', prompt: '清新自然，植物纹理，绿色调，森系风格，微距摄影' },
    { id: 4, src: 'https://images.unsplash.com/photo-1523821741446-edb2b68bb7a0?w=400&q=80', prompt: '科技感，赛博朋克，霓虹灯光，暗黑风格，故障艺术' },
    { id: 5, src: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=400&q=80', prompt: '阳光沙滩，热带风情，假日氛围，蓝天白云，广角镜头' },
    { id: 6, src: 'https://images.unsplash.com/photo-1470770841072-f978cf4d019e?w=400&q=80', prompt: '静谧山川，唯美风景，自然光，大气磅礴，国家地理风格' }
  ];
      ...
      <div className="flex-1 relative bg-[#F9FAFB] overflow-hidden flex">
                 ...
                 {elements.map((el) => {
                   ...
                   return (
                    ...
                    >
                      ...
                      {isSelected && (
                        <div 
                          className="absolute -top-14 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-white p-1.5 rounded-lg shadow-xl border border-black/5 cursor-default pointer-events-auto z-50"
                          onClick={(e) => e.stopPropagation()}
                          ...
                          {el.type === 'image' ? (
                            <>
                              <button 
                                ...
                                className="px-3 py-1.5 bg-gray-50 hover:bg-gray-100 text-xs font-medium text-gray-700 rounded-md transition-colors flex items-center gap-1.5 whitespace-nowrap"
                              >
                                <Upload size={14} />
                                本地上传
                              </button>
                              ...
                              <button 
                                ...
                                className="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-xs font-medium text-blue-600 rounded-md transition-colors flex items-center gap-1.5 whitespace-nowrap"
                              >
                                <Sparkles size={14} />
                                AI生图
                              </button>
        ...
        {showAIGenModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-xl shadow-2xl w-[90vw] h-[85vh] flex overflow-hidden">
              {/* Left Column: Prompt Input */}
              <div className="w-[360px] border-r border-gray-200 flex flex-col bg-gray-50/50">
                <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white">
                   <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                      <Sparkles className="text-accent-primary" size={20} />
                      AI 创意生图
                   </h3>
                   <button onClick={() => setShowAIGenModal(false)} className="text-gray-400 hover:text-gray-600 transition-colors">
                      <X size={20} />
                   </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                  ...
                  <div className="space-y-3">
                     <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                        灵感关键词
                        <span className="text-[10px] px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded-full">激发创意</span>
                     </label>
                     <div className="relative">
                        <select
                           value={aiGenInspirationCategory}
                           onChange={(e) => setAiGenInspirationCategory(e.target.value)}
                           className="w-full appearance-none bg-white border border-gray-200 rounded-lg px-3 py-2 text-xs font-medium text-gray-700 outline-none focus:border-black/20 focus:ring-2 focus:ring-black/5 transition-all"
                        >
                           {['通用', '人像', '风景', '二次元', '3D', '插画', '写实', '古风'].map(cat => (
                              <option key={cat} value={cat}>{cat}</option>
                           ))}
                        </select>
                        <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={14} />
                     </div>
                     <div className="bg-white p-3 rounded-xl border border-gray-200 min-h-[100px]">
                        <div className="flex flex-wrap gap-2">
                          {/* Mock keywords based on category */}
                          {['赛博朋克', '唯美', '高清', '大师作', '光影', '特写', '8k', '细腻', '梦幻', '极简', '未来感', '胶片感'].map(kw => (
                            <button 
                               key={kw}
                               onClick={() => setAiGenPrompt(prev => prev + (prev ? ' ' : '') + kw)}
                               className="px-2.5 py-1.5 bg-gray-50 hover:bg-gray-100 rounded-lg text-xs text-gray-600 border border-transparent hover:border-gray-200 transition-colors"
                            >
                               {kw}
                            </button>
                          ))}
              ...
              <div className="flex-1 bg-gray-100/50 flex flex-col relative overflow-hidden">
                 {/* Top Toolbar */}
                 <div className="absolute top-4 left-4 right-4 z-10 flex items-center gap-2 pointer-events-auto">
                    <div className="bg-white/80 backdrop-blur p-1 rounded-lg text-xs font-medium text-gray-500 shadow-sm flex gap-1">
                       <button className="px-3 py-1.5 rounded-md hover:bg-black/5 text-gray-600 transition-colors">风格参数</button>
                       <div className="w-px h-4 bg-gray-300 my-auto" />
                       <button className="px-3 py-1.5 rounded-md hover:bg-black/5 text-gray-600 transition-colors">高级参数</button>
                       <div className="w-px h-4 bg-gray-300 my-auto" />
                       <button 
                         onClick={() => setAiMiddleMode(aiMiddleMode === 'library' ? 'preview' : 'library')}
                         className={clsx(
                           "px-3 py-1.5 rounded-md transition-colors flex items-center gap-1.5",
                           aiMiddleMode === 'library' ? "bg-purple-100 text-purple-700" : "hover:bg-black/5 text-gray-600"
                         )}
                       >
                         <Package size={14} />
                         AI素材库
                 ...
                 <div className="flex-1 flex items-center justify-center p-8 pt-16">
                    {aiMiddleMode === 'library' ? (
                       selectedMaterial ? (
                          <div className="w-full h-full bg-white rounded-xl shadow-sm p-4 flex flex-col animate-in fade-in zoom-in-95 duration-200">
                             <div className="flex items-center justify-between mb-4">
                                <button onClick={() => setSelectedMaterial(null)} className="flex items-center gap-1 text-xs text-gray-500 hover:text-black transition-colors">
                                   <ChevronLeft size={14} />
                                   返回素材库
                                </button>
                                <span className="text-xs font-bold text-gray-900">素材预览</span>
                             </div>
                             <div className="flex-1 overflow-hidden rounded-lg bg-gray-50 flex items-center justify-center mb-4 border border-gray-100">
                                <img src={selectedMaterial.src} className="max-w-full max-h-full object-contain" />
                             </div>
                             <div className="space-y-3">
                                <div className="bg-gray-50 p-3 rounded-lg text-xs text-gray-600 leading-relaxed border border-gray-100">
                                   <span className="font-bold text-gray-900 mr-2">提示词:</span>
                                   {selectedMaterial.prompt}
                                </div>
                                <button 
                                  onClick={() => {
                                     setAiGenPrompt(selectedMaterial.prompt);
                                     setAiGenPreview(selectedMaterial.src);
                                     setAiMiddleMode('preview');
                                     setSelectedMaterial(null);
                                  }}
                                  className="w-full py-2.5 bg-black text-white rounded-xl text-sm font-medium hover:bg-gray-800 transition-colors flex items-center justify-center gap-2"
                                >
                                   <Sparkles size={16} />
                                   制作同款
                                </button>
                             ...
                             <div className="grid grid-cols-2 gap-4">
                                {materialLibrary.map(item => (
                                   <div 
                                     ...
                                     className="group cursor-pointer space-y-2"
                                   >
                                      <div className="aspect-video rounded-lg overflow-hidden border border-gray-200 bg-white relative shadow-sm hover:shadow-md transition-all">
                                         <img src={item.src} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                         <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
                                         <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 text-white text-[10px] px-2 py-1 rounded-full backdrop-blur-sm">
                                            点击预览
                                         </div>
                                         <div className="absolute top-2 left-2 bg-purple-500 text-white text-[10px] px-1.5 py-0.5 rounded-md shadow-sm">
                                            免抠
                                         </div>
                                      </div>
                                      <p className="text-xs text-gray-600 line-clamp-1 px-1 font-medium">{item.prompt}</p>
                                   </div>
                                ))}
                             </div>
                          </div>
                   ...
                   aiGenPreview ? (
                     <div className="relative w-full h-full flex items-center justify-center group">
                        <img src={aiGenPreview} className="max-w-full max-h-full object-contain rounded-lg shadow-2xl bg-white" alt="Generated" />
                        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0 bg-black/80 p-2 pl-4 rounded-full backdrop-blur-md shadow-2xl">
                           <span className="text-white/90 text-xs font-medium mr-2">满意这张图?</span>
                           <button 
                              onClick={() => {
                                 // Add to canvas
                           ...
                           <button className="p-2 hover:bg-white/20 rounded-full text-white transition-colors" title="下载">
                              <Download size={16} />
                           </button>
                        </div>
                     </div>
                   ) : (
                     <div className="text-center text-gray-400 space-y-6 animate-pulse">
                        <div className="w-32 h-32 bg-gray-200/50 rounded-full flex items-center justify-center mx-auto border-4 border-white shadow-sm">
                           <Sparkles size={48} className="text-gray-300" />
                        </div>
                        <div className="space-y-2">
                           <p className="text-base font-medium text-gray-500">等待生成...</p>
                           <p className="text-sm text-gray-400">在左侧输入提示词，让 AI 为你创造奇迹</p>
                        </div>
                     </div>
                   ))}
                 </div>
              ...
              <div className="w-72 border-l border-gray-200 bg-white flex flex-col">
                 <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                    <h4 className="text-sm font-medium text-gray-900">生成记录</h4>
                    <span className="text-xs text-gray-400">{aiGenHistory.length} 张</span>
                 </div>
                 <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                    {aiGenHistory.length > 0 ? (
                      <div className="grid grid-cols-2 gap-3">
                         {aiGenHistory.map((src, i) => (
                           <div 
                              key={i} 
                              className={clsx(
                                 "group relative aspect-square rounded-xl overflow-hidden cursor-pointer border-2 transition-all",
                                 aiGenPreview === src ? "border-black shadow-md ring-2 ring-black/5" : "border-transparent hover:border-gray-200"
                              )}
                              onClick={() => setAiGenPreview(src)}
                           >
                              <img src={src} className="w-full h-full object-cover transition-transform group-hover:scale-110" alt={`History ${i}`} />
                              {aiGenPreview === src && (
        ...
        <ExportModal
          ...
          onExport={(settings) => {
            if (previewImage) recordExportedAsset(previewImage, { source: 'MaterialEditor', export: settings });
